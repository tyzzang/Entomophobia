...
namespace BNG {
    public class DoorHelper : MonoBehaviour {
        ...
        void Update() {
            ...
            if (DegreesTurned > DegreesTurnToOpen)
            {
                Vector3 _pos = new Vector3(DoorLockTransform.forward.x, 0, DoorLockTransform.forward.z).normalized;
                Vector3 _userPos = new Vector3(GManager.Instance.IsUserObj.transform.position.x, 0, GManager.Instance.IsUserObj.transform.position.z);
                Vector3 _doorPos = new Vector3(DoorLockTransform.position.x, 0, DoorLockTransform.position.z);
                Vector3 _val = (_userPos - _doorPos).normalized;
                float _dot = Vector3.Dot(_pos, _val);
               
                if (_dot < 0)
                {
                    transform.DOLocalRotate(OpenAngle[0], 1.5f, RotateMode.Fast);
                }
                else if(_dot > 0)
                {
                    transform.DOLocalRotate(OpenAngle[1], 1.5f, RotateMode.Fast);
                }
            }
